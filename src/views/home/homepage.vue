            ...
            <div class="flex-content">
              <div class="task">
                <div class="task-item task-first-item">
                  ...
                  <div class="task-item-label">{{ state.resource.all }}%</div>
                </div>
                <div class="task-item">
                  <div class="task-item-box task1">
                    <div class="task-item-value">CPU</div>
                    <div class="task-item-label">{{ state.resource.cpu }}%</div>
                  </div>
                </div>
                <div class="task-item">
                  <div class="task-item-box task2">
                    <div class="task-item-value">内存</div>
                    <div class="task-item-label">{{ state.resource.memory }}%</div>
                  </div>
                </div>
                <div class="task-item">
                  <div class="task-item-box task3">
                    <div class="task-item-value">硬盘</div>
                    <div class="task-item-label">{{ state.resource.disk }}%</div>
                  </div>
                </div>
                <div class="task-item">
                  <div class="task-item-box task4">
                    <div class="task-item-value">网络</div>
                    <div class="task-item-label">{{ state.resource.network }}%</div>
                  </div>
                </div>
              </div>
              <div class="progress">
                <div class="progress-item">
                  ...
                  <div class="progress-box">
                    <el-progress :percentage=parseInt(state.resource.cpu) color="#43bdf0"></el-progress>
                  </div>
                </div>
                <div class="progress-item">
                  <span>内存使用率</span>
                  <div class="progress-box">
                    <el-progress :percentage=parseInt(state.resource.memory) color="#43bdf0"></el-progress>
                  </div>
                </div>
                <div class="progress-item">
                  <span>硬盘使用率</span>
                  <div class="progress-box">
                    <el-progress :percentage=parseInt(state.resource.disk) color="#43bdf0"></el-progress>
                  </div>
                </div>
                <div class="progress-item">
                  <span>网络使用率</span>
                  <div class="progress-box">
                    <el-progress :percentage=parseInt(state.resource.network) color="#43bdf0"></el-progress>
                  </div>
                </div>
              </div>
            </div>
  ...
  homeThree: [
    {
      ...
      label: '网口管理',
      value: '/sysmanage/address/networkinterface',
      iconColor: '#F72B3F',
    },
    {
      icon: 'iconfont icon-zhongyingwenqiehuan',
      label: '串口管理',
      value: '/sysmanage/address/serial',
      iconColor: '#91BFF8',
    },
    {
      icon: 'iconfont icon--chaifenhang',
      label: '封装应用头',
      value: '/sysmanage/package',
      iconColor: '#88D565',
    },
    {
      icon: 'iconfont icon-shuxingtu',
      label: '消息头对象',
      value: '/sysmanage/messheader',
      iconColor: '#88D565',
    },
    {
      icon: 'iconfont icon-ziti',
      label: '消息体对象',
      value: '/sysmanage/messbody',
      iconColor: '#FBD4A0',
    },
    {
      icon: 'iconfont icon-neiqianshujuchucun',
      label: '消息转化列表',
      value: '/sysmanage/messtraslate',
      iconColor: '#FBD4A0',
    },
    {
      icon: 'iconfont icon--chaifenhang',
      label: '数据域集合',
      value: '/sysmanage/field',
      iconColor: '#FBD4A0',
    },
    {
...
const initPieChart = () => {
  ...
  var getname = ['翼龙系列标准', '深潜专用标准', '通用设备', '固体火箭系列标准', '固定翼无人机'];
  var getvalue = [34.2, 38.87, 17.88, 9.05, 2.05];
  getname=state.pie.name;
  getvalue=state.pie.value;
  var data = [];
...
const initBarChart = () => {
  ...
  const option = {
    ...
    tooltip: {trigger: 'axis'},
    legend: {data: ['新增用户数', '登录数', '更新字段数'], right: 0},
    grid: {top: 70, right: 80, bottom: 30, left: 80},
    xAxis: [
      {
        type: 'category',
        data:state.bar.date1,
        boundaryGap: true,
        axisTick: {show: false},
      },
    ],
  ...
  homeApi().getData(
      ...
      .then(res => {
        ...
        if (res.code == '200') {
          ...
          state.resource=res.data.resource;
          state.homeOne[0].num1=res.data.one.flownums;
          state.homeOne[0].num2=res.data.one.flowadd;
          state.homeOne[1].num1=res.data.one.fieldsnums;
          state.homeOne[1].num2=res.data.one.fieldsadd;

          state.homeOne[2].num1=res.data.one.diswnums;
          state.homeOne[2].num2=res.data.one.disadd;
          state.homeOne[3].num1=res.data.one.usernums;
          state.homeOne[3].num2=res.data.one.useradd;
