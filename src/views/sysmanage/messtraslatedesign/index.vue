        ...
        <div class="workflow-content">
          ...
          <div id="workflow-right" class="workflow-right" ref="workflowRightRef">
            <el-table :data="state.tableDataSource.data" row-key="ID"
                      ...
                      default-expand-all class="eltableclass" id="sourcetable" :style="state.tableStyle">
...
-->
    ...
    <!-- 编辑窗口 -->
    <UserDialog ref="userDialogRef" @refresh="getTableDataTranslate()" @delConn="delConn"/>
    <!-- 顶部工具栏-帮助弹窗 -->
    <Help ref="helpRef"/>
  </div>
</template>
...
const initJsPlumb = () => {
  jsPlumb.ready(() => {
    ...
    // 连线出现偏差时，特殊处理。
    state.jsPlumb.bind('connectionAborted', (conn: any, originalEvent) => {
      // console.log("目标节点为空");
      const {sourceId, targetId} = conn;
...
const handleWheel = (event) => {
  ...
  var origin_top = state.tableStyle.top.replace(/px/, "");
  var top = 50;
  if (deltaY < 0) {
...
const delConn = (conn) => {
  ...
  let sourceids = "source_" + conn.sourceId;
  let targetids = "target_" + conn.targetId;

  var connf = state.jsPlumb.getConnections({
