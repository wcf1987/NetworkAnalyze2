...
const initJsPlumb = () => {
  ready(() => {
    ...
    state.jsPlumb.bind(EVENT_CONNECTION_MOUSEDOWN, (connection) => {
      //console.log("EVENT_CONNECTION_MOUSEOVER", connection);
      repaintWithScroll()
    });
    state.jsPlumb.bind(EVENT_CONNECTION_MOVED, (connection) => {
      //console.log("EVENT_CONNECTION_MOUSEOVER", connection);
      repaintWithScroll()
    });
    state.jsPlumb.bind(EVENT_ENDPOINT_MOUSEOVER, (connection) => {
      //console.log("EVENT_CONNECTION_MOUSEOVER", connection);
      repaintWithScroll()
    });
    state.jsPlumb.bind(EVENT_ENDPOINT_MOUSEOUT, (connection) => {
      //console.log("EVENT_CONNECTION_MOUSEOVER", connection);
      repaintWithScroll()
    });

    state.jsPlumb.bind(EVENT_CONNECTION_MOUSEOVER, (connection: Connection) => {
      //console.log("EVENT_CONNECTION_MOUSEOVER", connection);
      repaintWithScroll()
    });
    state.jsPlumb.bind(EVENT_CONNECTION_MOUSEOUT, (connection: Connection) => {
      //console.log("EVENT_CONNECTION_MOUSEOUT", connection);
      repaintWithScroll()
    });
    state.jsPlumb.bind(EVENT_DRAG_START, (p) => {
      repaintWithScroll()
    })
    state.jsPlumb.bind(EVENT_DRAG_MOVE, (p) => {
      console.log("drag....");
      repaintWithScroll()
    })
    // 点击线弹出右键菜单
    state.jsPlumb.bind(EVENT_CONNECTION_CONTEXTMENU, (conn: any, originalEvent: MouseEvent) => {
    ...
    state.jsPlumb.bind(INTERCEPT_BEFORE_DRAG, (params: BeforeDragParams<Element>) => {
      repaintWithScroll()
          console.log('before drag interceptor', params);
        }
    );
    // 连线时
    state.jsPlumb.bind(EVENT_CONNECTION, (connectionEstablishedParams) => {
      console.log(connectionEstablishedParams);
      const {sourceId, targetId} = connectionEstablishedParams;
...
const initLeaf = (row, type) => {
  ...
  if (type == 'Source') {
    ...
    elem.id = "source_" + elem.classList[2];
    ins.addEndpoint(elem, state.jsplumbMakeSource2)
    ...
    elem.id = "target_" + elem.classList[2];
    ins.addEndpoint(elem, state.jsplumbMakeTarget2)
  }
          ...
          var connection = state.jsPlumb.connect(
              {
                ...
                connector: state.jsplumbConnect2,
                paintStyle: state.paintStyle2,
                hoverPaintStyle: state.hoverPaintStyle2,
                overlays: [
  ...
  .workflow {
    ...
    .workflow-content {
      ...
      .workflow-right {
        ...
        :deep(svg.sourceEndpoint circle) {
        ...
        :deep(svg.targetEndpoint circle) {
          background-color: white;
          fill: #1879ffa1;
          stroke: blue;
          margin-right: 7px;
