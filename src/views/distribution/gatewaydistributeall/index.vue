...
const delPoint = (flowid) => {
  for (let v = 0; v < state.tableDataFlowShow.length; v++) {
    if (state.tableDataFlowShow[v].ID == flowid) {
      console.log(state.tableDataFlowShow[v])
      state.tableDataFlowShow.splice(v, 1);
      break
    }
  }
  console.log(state.tableDataDistribute.data);
  for (let v = 0; v < state.tableDataDistribute.data.length; v++) {
    if (state.tableDataDistribute.data[v].FlowID == flowid && state.tableDataDistribute.data[v].GateID == state.gateway.ID) {

      state.tableDataDistribute.data.splice(v, 1);
      return
    }
  }
}
